...
def upsert_chat(chat_id: int, title: str, chat_type: str, enabled: bool=True):
    with psycopg2.connect(DB_DSN) as conn:
        with conn.cursor() as cur:
            ...
            VALUES (%s, %s, %s, %s, now())
            ON CONFLICT (chat_id)
            DO UPDATE SET title=EXCLUDED.title, type=EXCLUDED.type, is_enabled=EXCLUDED.is_enabled, updated_at=now()
...
def set_enabled(chat_id: int, enabled: bool):
    with psycopg2.connect(DB_DSN) as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE telegram_chats SET is_enabled=%s, updated_at=now() WHERE chat_id=%s", (enabled, chat_id))
        conn.commit()
...
async def register(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Registered this chat for reports.\n"
        f"Chat: {title}\n"
        f"(No need to set chat_id manually anymore.)"
    )
